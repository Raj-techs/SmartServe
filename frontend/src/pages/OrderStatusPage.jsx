...
import { getShopOrders, markOrderPaid, leaveSession } from '../services/api';
import { getSocket, whenConnected } from '../hooks/useSocket';
import toast from 'react-hot-toast';
import CashPaymentScreen from '../components/common/CashPaymentScreen';

const PHASES = [
    { key: 'pending_waiter', label: 'Waiting for Waiter', color: 'bg-slate-500',   text: 'text-slate-300',   border: 'border-slate-500/40',   bg: 'bg-slate-500/10'   },
    { key: 'rejected',       label: 'Order Cancelled',    color: 'bg-red-500',     text: 'text-red-400',     border: 'border-red-500/40',     bg: 'bg-red-500/10'     },
...
export default function OrderStatusPage() {
    ...
    const [phase, setPhase] = useState('placed');
    ...
    const fetchOrders = useCallback(async () => {
        try {
            ...
            // ── SESSION ISOLATION: filter by BOTH tableNumber AND sessionTokenId ──
            const mine = data.filter(o =>
                o.tableNumber === tableNumber &&
                (!session.tokenId || o.sessionTokenId === session.tokenId)
            );
            setOrders(mine);
            if (mine.length === 0) return;
            if (mine.every(o => o.paymentStatus === 'paid')) { setPhase('paid'); return; }
            const myItemsAll = mine.flatMap(o => o.items || []).filter(i => i.username === session.username);
            if (myItemsAll.length === 0) return;
            if (myItemsAll.every(i => i.status === 'served'))         { setPhase('served');         return; }
            if (myItemsAll.some(i => i.status === 'ready'))            { setPhase('ready');           return; }
            if (myItemsAll.some(i => i.status === 'preparing'))        { setPhase('preparing');       return; }
            if (myItemsAll.some(i => i.status === 'received'))         { setPhase('placed');          return; }
            if (myItemsAll.every(i => i.status === 'cancelled'))       { setPhase('rejected');        return; }
            if (myItemsAll.some(i => i.status === 'pending_waiter'))   { setPhase('pending_waiter');  return; }
            setPhase('placed');
        } catch { /* silent */ }
        finally { setLoading(false); }
    }, [shopId, tableNumber, session.tokenId, session.username]);
    ...
    useEffect(() => {
        ...
        socket.on('order_status_changed', ({ phase: p, username }) => {
            if (!username || username === session.username) {
                setPhase(p);
                fetchOrders();
                if (p === 'pending_waiter') toast('🛎️ Waiter is coming to verify your order...', { icon: '⏳' });
                if (p === 'rejected')       toast.error('Order was cancelled by waiter. Please reorder.');
                if (p === 'placed')         toast.success('✅ Waiter verified! Order sent to kitchen.');
            }
        });
        const poll = setInterval(fetchOrders, 10000);

        // ── Tab/App close cleanup ──────────────────────────────────
        const handleUnload = () => {
            const sess = JSON.parse(localStorage.getItem('customer_session') || '{}');
            if (sess.tokenId && sess.username) {
                const base = (import.meta.env.VITE_API_URL || 'http://localhost:5000/api');
                navigator.sendBeacon(
                    `${base}/order/cancel-user-items`,
                    new Blob([JSON.stringify({ sessionTokenId: sess.tokenId, username: sess.username })],
                        { type: 'application/json' })
                );
                navigator.sendBeacon(
                    `${base}/token/leave/${sess.tokenId}/${encodeURIComponent(sess.username)}`,
    ...
    // ── MY ITEMS ONLY (session isolated + username filtered) ──
    const allItems = orders.flatMap(o => o.items || []);
    const myItems  = allItems.filter(i => i.username === session.username);
    // ── MY TOTAL: only items ordered by this user ──
    const myTotal  = myItems.reduce((a, i) => {
    ...
    const handlePayOnline = async () => {
        ...
        setPaying(true);
        try {
            await markOrderPaid(activeOrder._id, 'online');
            getSocket().emit('payment_done', { shopId, tableNumber });
            toast.success('Payment successful! Thank you 🙏');
            setPhase('paid');
            await fetchOrders();
            // Clean up session and go to safe end screen
            try {
                if (session?.tokenId && session?.username) {
                    const { leaveSession } = await import('../services/api');
    ...
    const handlePayCash = async () => {
        // Re-fetch latest so myTotal is current before sending to waiter
        try {
            const { data } = await getShopOrders(shopId);
            const latestMine = (data || []).filter(o =>
                o.tableNumber === tableNumber &&
                (!session.tokenId || o.sessionTokenId === session.tokenId)
            );
            const latestItems = latestMine
                .flatMap(o => o.items || [])
                .filter(i => i.username === session.username);
            const latestTotal = latestItems.reduce(
                (a, i) => a + ((i.productId?.price || 0) * i.quantity), 0
            );
            const latestOrder = latestMine.find(o => o.status === 'active');

            whenConnected((socket) => {
                socket.emit('cash_payment_request', {
                    shopId,
                    tableNumber,
            ...
            // Update local state too so CashPaymentScreen shows correct amount
            setOrders(latestMine);
        } catch {
            // Fallback: emit with whatever myTotal we have
            whenConnected((socket) => {
                socket.emit('cash_payment_request', {
                    shopId, tableNumber,
                    username: session.username,
    ...
    const handleCashDone = () => {
        ...
        setPhase('paid');
        fetchOrders();
        // CashPaymentScreen already navigates to /session-ended via its own handler
        // This is just a fallback reset of local state
    };

    // Show full-screen cash payment UI
    if (showCashScreen) {
        return (
            <CashPaymentScreen
                session={session}
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-white flex flex-col max-w-md mx-auto relative overflow-hidden pb-4">
            ...
            {/* Floating Status Bar */}
            {myItems.length > 0 && (
                <motion.div key={phase} initial={{ y: -40, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
                    ...
                    <div className="flex gap-1">
                        {PHASES.map((p, i) => {
                            ...
                            return <div key={p.key} className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${idx >= i ? currentPhase.color : 'bg-slate-700'}`} />;
                        })}
                    </div>
                </motion.div>
            )}
            ...
            <div className="flex items-center justify-between px-5 pt-5 pb-3">
                ...
                <button onClick={() => navigate(`/menu/${shopId}/${tableNumber}`)}
                    className="flex items-center gap-1.5 text-xs font-bold text-blue-400 bg-blue-500/10 border border-blue-500/20 px-3 py-1.5 rounded-full">
            ...
            {myItems.length > 0 && (
                <div className="flex flex-col items-center py-2">
                    ...
                    <p className="text-lg font-bold text-white mb-4">
                        {phase === 'pending_waiter' && 'Waiter is coming to verify... ⏳'}
                        {phase === 'rejected'       && 'Order cancelled. Please reorder 🔄'}
                    ...
                    <div className="relative flex items-center justify-center" style={{ width: 280, height: 280 }}>
                        <div className="absolute w-28 h-28 rounded-full border border-blue-500/30" style={{ animation: 'sonarPulse 4s ease-out infinite' }} />
                        <div className="absolute w-28 h-28 rounded-full border border-blue-500/20" style={{ animation: 'sonarPulse 4s ease-out infinite 2s' }} />
                        <div className="absolute w-52 h-52 rounded-full border border-dashed border-white/10" style={{ animation: 'spinOrbit 15s linear infinite' }}>
                                        ...
                                        <div style={{ animation: 'counterSpin 15s linear infinite' }}>
                                            <div className="w-12 h-12 bg-slate-800 border-2 border-blue-500/60 rounded-2xl overflow-hidden shadow-lg relative">
                                                {item.productId?.image
                                                    ...
                                                    : <div className="w-full h-full flex items-center justify-center text-xl">{FOOD_ICONS[i % FOOD_ICONS.length]}</div>
                                                }
                                                <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-full border-2 border-slate-950">×{item.quantity}</span>
                                            </div>
                                        </div>
                                    </div>
                                );
                        ...
                        <div className="absolute w-20 h-20 rounded-full flex items-center justify-center border-4 border-slate-900 z-10"
                            style={{ background: 'linear-gradient(135deg,#2563eb,#1d4ed8)', boxShadow: '0 0 40px rgba(37,99,235,0.5)' }}>
                            <span className="text-3xl">🔥</span>
                        </div>
                    </div>
                    <style>{`
                        @keyframes sonarPulse { 0%{transform:scale(1);opacity:0.7;} 100%{transform:scale(3.2);opacity:0;} }
            ...
            <div className="flex overflow-x-auto gap-4 px-5 py-3 scrollbar-hide">
                {[
                    ...
                    { step: 3, color: 'text-purple-400', label: 'STEP 3: ADD ON',  body: 'Want more? Tap "+ Add More" to go back to the menu.' },
                    { step: 4, color: 'text-green-400',  label: 'STEP 4: PAYMENT', body: 'Choose Cash or Online. Waiter collects cash at your table.' },
                ].map(c => (
            ...
            {phase === 'rejected' && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                    className="mx-4 mb-3 bg-red-500/10 border border-red-500/30 rounded-2xl p-4 flex items-center gap-3">
                    <span className="text-2xl">❌</span>
                    <div>
                        <p className="text-red-400 font-bold">Order was cancelled by waiter</p>
                        <p className="text-slate-400 text-xs mt-0.5">You can place a new order from the menu.</p>
                    </div>
                </motion.div>
            )}
            ...
            <div className="px-4 space-y-2 mt-2">
                {myItems.length === 0 && (
                    <div className="text-center py-10">
                        ...
                        <button onClick={() => navigate(`/menu/${shopId}/${tableNumber}`)}
                            className="mt-3 bg-blue-500 text-white px-6 py-3 rounded-xl font-bold text-sm">Browse Menu</button>
                    </div>
                )}
                {myItems.map((item, idx) => {
                    const sMap = {
                        pending_waiter: { label: 'Pending Waiter', cls: 'text-slate-300 bg-slate-500/10 border-slate-500/30' },
                    ...
                    const s = sMap[item.status] || sMap.received;
                    return (
                        <motion.div key={idx} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-slate-500">Qty: {item.quantity} • ₹{item.productId?.price * item.quantity}</p>
                            </div>
            ...
            <AnimatePresence>
                {(phase === 'ready' || phase === 'served') && !confirmedReceived && (
                    <motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }} className="px-4 mt-4">
                        <button onClick={handleOrderReceived}
                            ...
                            <CheckCircle2 className="w-6 h-6" /> ORDER RECEIVED
                        </button>
                        <p className="text-center text-slate-500 text-xs mt-2">Confirm your food has arrived</p>
                    </motion.div>
                )}
                ...
                <div className="px-4 mt-4 mb-4">
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                        <div className="flex justify-between items-center mb-4">
                            <div>
                                <span className="text-slate-400 font-semibold">Your Bill</span>
                                <p className="text-xs text-slate-500 mt-0.5">{session?.username}'s items only</p>
                            </div>
                            <span className="text-2xl font-extrabold text-white">₹{myTotal}</span>
                        </div>
                        {confirmedReceived ? (
                            <div className="grid grid-cols-2 gap-3">
                                <button onClick={handlePayCash} disabled={paying}
                                    className="py-3.5 rounded-2xl font-bold border-2 border-slate-700 text-slate-300 hover:border-orange-500 hover:text-orange-400 transition disabled:opacity-50 text-sm">
                                    💵 Pay Cash
                                </button>
                                <button onClick={handlePayOnline} disabled={paying}
                                    className="py-3.5 rounded-2xl font-bold bg-blue-600 hover:bg-blue-500 text-white shadow-lg disabled:opacity-50 flex items-center justify-center gap-2 text-sm transition">
                            ...
                            <div className="bg-slate-800/60 rounded-xl p-3 text-center">
                                <p className="text-slate-500 text-xs font-medium">Payment options appear after confirming food received</p>
                            </div>
                        )}
                    </div>
                </div>
